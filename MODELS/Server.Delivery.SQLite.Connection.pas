...
begin
  ...
  FConnection.ExecSQL('CREATE TABLE IF NOT EXISTS PEDIDOS_TIPOS_PAGAMENTOS (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL,' + ' ID_PEDIDO INTEGER REFERENCES PEDIDOS (ID) ON DELETE CASCADE ON UPDATE CASCADE, ID_TIPO_PAGAMENTO INTEGER REFERENCES TIPOS_PAGAMENTO (ID) ON DELETE CASCADE ON UPDATE CASCADE);');
