...
begin
  ...
  FConnection.ExecSQL('CREATE TABLE IF NOT PEDIDOS (ID INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, DATA DATETIME NOT NULL DEFAULT (CURRENT_TIMESTAMP), TOTAL DECIMAL (10, 2) NOT NULL DEFAULT (0.0),' + ' CANCELADO BOOLEAN NOT NULL DEFAULT (false), ABERTO BOOLEAN NOT NULL DEFAULT (true), OBS TEXT, TIPO_PAGAMENTO INTEGER REFERENCES TIPOS_PAGAMENTO (ID)' + ' DEFAULT NULL, CAIXA INTEGER NOT NULL REFERENCES CAIXAS (ID) ON DELETE CASCADE, CLIENTE INTEGER REFERENCES CLIENTES (id) DEFAULT NULL, ENDERECO_ENTREGA INTEGER REFERENCES ENDERECOS (ID) DEFAULT NULL);');
